...
Goal :
  ...
  Depend on P(s-1) values that are pulled from "/output/pagerank/prob_sc.txt"
...
Execution :
  Launched as :
      hadoop jar <path_tohadoop_dir>/share/hadoop/tools/lib/hadoop-streaming-2.7.3.jar \
	      -input /input/pagerank/adjacent_list_v \
...
with open(input_dir + '/adjacent_list_v', "r") as f:
    n = 1 + int((f.readlines()[-1].split(':'))[0])


'''
- building  P(s-1) vector from last line in "prob_sc.txt"
'''
if os.path.isfile(output_dir + '/prob_sc.txt'):
	with open(output_dir + '/prob_sc.txt', "r") as f:
...
'''
- building outcoming_links_number from adjacent_list
outcoming_links_number: # of outcoming links "nj" for each node j
'''
outcoming_links_number = []
for line in open(input_dir + '/adjacent_list', "r"):
...
'''
- building Tmat (T matrix) from stdin input (line in adjacent_list_v)
...
########################## for line in open(input_dir + 'adjacent_list_v', "r"):
	# building Tmat
